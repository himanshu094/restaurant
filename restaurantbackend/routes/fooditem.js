...
router.post('/fooditem_submit',upload.any(), function(req, res, next) {
  ...
  router.post('/fetch_all_fooditem',function(req,res){
    pool.query('select F.*, (select C.categoryname from category C where C.categoryid=F.categoryid) as categoryname from fooditems F where restaurantid=?',[req.body.restaurantid],function(error,result){
        if(error)
        {
            console.log(error)
