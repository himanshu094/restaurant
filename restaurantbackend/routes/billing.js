...
router.post('/fetch_total',function(req,res){
  ...
  pool.query('select round(sum(totalamount),2) as totalbill from billing where billdate between ? and ?',[req.body.fromdate,req.body.tilldate],function(error,result){
      if(error)
      {
          console.log(error)
  ...
  router.post('/fetch_totalsale_month',function(req,res){
    ...
    pool.query('select month(billdate) as month, round(sum(totalamount),2) as totalbill from billing group by month(billdate) order by billdate',function(error,result){
        if(error)
        {
          console.log(error)
          res.status(200).json({status:false,message:'Database Error',data:[]});
